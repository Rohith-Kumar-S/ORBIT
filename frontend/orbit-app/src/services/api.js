...
export const authAPI = {
  login: async (email, password) => {
    ...
    const response = await api.post('http://localhost:8080/api/auth/user/login', {
      email,
      password
    });
  ...
  register: async (name, mobile, email, password) => {
    ...
    const response = await api.post('http://localhost:8080/api/auth/user/register', {
      name,
      mobileNumber: mobile,
      email,
      password
...
export const ordersAPI = {
  getOrders: async () => {
    // Dummy implementation
    const response = await api.get('http://localhost:8082/api/order');
